-- CAT HUB GUI integration
-- Uses the user's CAT HUB GUI library instead of the original built-in Instance/Rivals UI library.
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/CatJackop/CATHUBGUI/refs/heads/main/CAT_HUB_GUI.luau"))()

local Options = {}
local Toggles = {}

local function registerControl(index, control, kind)
    if index and control then
        if kind == "toggle" then
            Toggles[index] = control
        else
            Options[index] = control
        end
    end
    return control
end

local function decorateControl(control, index, kind)
    if not control then return control end
    registerControl(index, control, kind)

    -- Compatibility helpers used by the original Instance/Rivals code.
    function control:AddColorPicker(_, _)
        return self
    end

    function control:AddKeyPicker(name, info)
        info = info or {}
        local key = {
            Type = "KeyPicker",
            Name = name,
    ...
    function control:SetValues(values)
        if self.Values then self.Values = values end
        if self.SetOptions then pcall(self.SetOptions, self, values) end
        return self
    end
...
local function makeInfo(info, index)
    info = info or {}
    local t = {}
    for k,v in pairs(info) do t[k]=v end
    t.Name = t.Name or t.Text or index
    t.Flag = t.Flag or index
    return t
end
...
local function wrapTab(rawTab)
    ...
    function proxy:AddToggle(index, info)
        info = makeInfo(info, index)
        local c = rawTab:AddToggle(info)
        return decorateControl(c, index, "toggle")
    end

    function proxy:AddButton(index, info)
        if type(index) == "table" then
            info = index
            index = info.Name or info.Text or info.Title
        end
        info = info or {}
        local cfg = makeInfo(info, index)
        local c = rawTab:AddButton(cfg)
        return decorateControl(c, index, "button")
    end

    function proxy:AddSlider(index, info)
        info = makeInfo(info, index)
        if info.Increment == nil then
            local r = tonumber(info.Rounding)
            info.Increment = r and (10 ^ (-r)) or 1
        end
        local c = rawTab:AddSlider(info)
        return decorateControl(c, index, "slider")
    end

    function proxy:AddDropdown(index, info)
        info = makeInfo(info, index)
        info.Options = info.Options or info.Values
        info.MultiSelect = info.MultiSelect ~= nil and info.MultiSelect or info.Multi
        local c = rawTab:AddDropdown(info)
        return decorateControl(c, index, "dropdown")
    end

    function proxy:AddInput(index, info)
        info = makeInfo(info, index)
        local c = rawTab:AddTextBox(info)
        return decorateControl(c, index, "input")
    end

    function proxy:AddTextBox(index, info)
        return self:AddInput(index, info)
    end

    function proxy:AddLabel(text)
        local c = rawTab:AddParagraph(tostring(text or ""))
        return decorateControl(c, nil, "label")
    end

    function proxy:AddBlank(_)
        return self
    end
    ...
    function proxy:AddDependencyBox()
        local dep = {}
        function dep:SetupDependencies(_) return self end
        function dep:AddToggle(...) return proxy:AddToggle(...) end
        function dep:AddSlider(...) return proxy:AddSlider(...) end
        function dep:AddDropdown(...) return proxy:AddDropdown(...) end
        function dep:AddInput(...) return proxy:AddInput(...) end
        function dep:AddButton(...) return proxy:AddButton(...) end
        function dep:AddColorPicker(...) return self end
        function dep:AddKeyPicker(...) return self end
        function dep:AddDependencyBox(...) return self end
        return dep
    end

    function proxy:AddDualSlider(leftIndex, rightIndex, leftInfo, rightInfo)
        local left = self:AddSlider(leftIndex, leftInfo)
        self:AddSlider(rightIndex, rightInfo)
        return left
    end

    function proxy:AddLeftGroupbox(_) return self end
    function proxy:AddRightGroupbox(_) return self end

    local function tabboxProxy()
        local box = {}
        function box:AddTab(_) return box end
        function box:AddToggle(...) return proxy:AddToggle(...) end
        function box:AddSlider(...) return proxy:AddSlider(...) end
        function box:AddDropdown(...) return proxy:AddDropdown(...) end
        function box:AddInput(...) return proxy:AddInput(...) end
        function box:AddTextBox(...) return proxy:AddTextBox(...) end
        function box:AddButton(...) return proxy:AddButton(...) end
        function box:AddLabel(...) return proxy:AddLabel(...) end
        function box:AddDependencyBox(...) return proxy:AddDependencyBox(...) end
        function box:AddDualSlider(...) return proxy:AddDualSlider(...) end
        function box:AddColorPicker(...) return box end
        function box:AddKeyPicker(...) return box end
        return box
    end

    function proxy:AddLeftTabbox(_) return tabboxProxy() end
    function proxy:AddRightTabbox(_) return tabboxProxy() end

    return proxy
end

-- The CAT HUB library uses MakeWindow/MakeTab rather than CreateWindow/AddTab.
local Window = Library:MakeWindow({
    Title = "CAT HUB",
    SubTitle = "Instance Rivals",
    ScriptFolder = "cat_hub_instance_rivals",
})

Library.ScreenGui = (gethui and gethui():FindFirstChild("redz-library-v5")) or game:GetService("CoreGui"):FindFirstChild("redz-library-v5")
Library.Unloaded = false
Library.AccentColor = Color3.fromRGB(246, 248, 98)
Library.MainColor = Color3.fromRGB(28, 28, 30)
Library.BackgroundColor = Color3.fromRGB(25, 25, 25)
Library.OutlineColor = Color3.fromRGB(45, 45, 45)
Library.FontColor = Color3.fromRGB(255, 255, 255)
Library.Font = Enum.Font.BuilderSans

function Library:Notify(text, time)
    pcall(function()
        local tab = Window:GetTabByTitle("notifications")
        if not tab then tab = Window:MakeTab({Name="notifications"}) end
        tab:AddParagraph(tostring(text), "CAT HUB")
    end)
end
...
function Library:SetWatermarkVisibility(_) end
function Library:SetWatermark(_) end
function Library:UpdateDependencyBoxes() end
function Library:AddToRegistry(...) end
function Library:OnUnload(fn) self._UnloadCallback = fn end
function Library:Unload()
    self.Unloaded = true
    if self._UnloadCallback then pcall(self._UnloadCallback) end
    pcall(function() self:Destroy() end)
end
function Library:ToEnum(enum, value)
    if typeof(value) == "EnumItem" then return value end
    local ok, result = pcall(function() return enum[value] end)
...
function Window:AddTab(name)
    return wrapTab(self:MakeTab({Name=name}))
end

function Window:AddOutsideButton(config)
    return self:CreateMobileMinimizer({
...
-- Movable CAT HUB logo/minimizer button.
local CAT_HUB_LOGO = "rbxassetid://126031329785796"
local logoButton = Window:CreateMobileMinimizer({
    Image = CAT_HUB_LOGO,
    Size = UDim2.fromOffset(48,48),
    Position = UDim2.fromScale(0.17,0.28),
    BackgroundColor3 = Color3.fromRGB(246,248,98),
})
getgenv().CATHUBHideShowButton = logoButton

-- CAT HUB startup notification.
pcall(function()
    local tab = Window:GetTabByTitle("info") or Window:MakeTab({Name="info"})
    tab:AddParagraph("CAT HUB loaded", "Instance Rivals")
end)
